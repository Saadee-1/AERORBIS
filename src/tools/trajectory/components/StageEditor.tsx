    ...
    <AeroCard title="Rocket Stages" icon={Rocket}>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <p className="text-sm text-gray-400">Configure rocket stages (first stage = bottom)</p>
          <Button
            onClick={addStage}
            variant="outline"
            size="sm"
        ...
        {stages.map((stage, stageIndex) => (
          <div key={stage.id} className="p-4 bg-slate-800/30 rounded-lg border border-cyan-400/20">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <AeroFormField label="Dry Mass" helperText="kg" htmlFor={`dry-${stageIndex}`}>
                <Input
                  ...
                  type="number"
                  value={stage.dryMass}
                  onChange={(e) => updateStage(stageIndex, 'dryMass', parseFloat(e.target.value) || 0)}
              ...
              <AeroFormField label="Fuel Mass" helperText="kg" htmlFor={`fuel-${stageIndex}`}>
                <Input
                  ...
                  onChange={(e) => updateStage(stageIndex, 'fuelMass', parseFloat(e.target.value) || 0)}
                  className="bg-slate-700/50 border-cyan-400/30 text-white"
                  min="0"
                  step="100"
                />
              ...
              <AeroFormField label="Drag Coefficient" helperText="Cd" htmlFor={`cd-${stageIndex}`}>
                <Input
                  ...
                  type="number"
                  value={stage.Cd}
                  onChange={(e) => updateStage(stageIndex, 'Cd', parseFloat(e.target.value) || 0)}
                  className="bg-slate-700/50 border-cyan-400/30 text-white"
                  min="0"
                  max="2"
                  step="0.01"
              ...
              <AeroFormField label="Frontal Area" helperText="m²" htmlFor={`area-${stageIndex}`}>
                <Input
                  ...
                  type="number"
                  value={stage.area}
                  onChange={(e) => updateStage(stageIndex, 'area', parseFloat(e.target.value) || 0)}
                  className="bg-slate-700/50 border-cyan-400/30 text-white"
                  min="0"
                  step="0.1"
                />
            ...
            <div className="border-t border-cyan-400/20 pt-4">
              <div className="flex justify-between items-center mb-3">
                <Label className="text-sm font-semibold text-cyan-400">Engines</Label>
                <Button
                  onClick={() => addEngine(stageIndex)}
                  variant="outline"
                  size="sm"
                  className="border-cyan-400/40 text-cyan-400"
                >
              ...
              {stage.engines.map((engine, engineIndex) => (
                <div key={engineIndex} className="p-3 bg-slate-700/30 rounded mb-2">
                  <div className="grid grid-cols-4 gap-2">
                    <AeroFormField label="Engine" helperText="" htmlFor={`engine-${stageIndex}-${engineIndex}`}>
                      <Select
                        value={engine.id}
                        onValueChange={(value) => updateStageEngine(stageIndex, engineIndex, 'id', value)}
                      >
                        <SelectTrigger className="bg-slate-600/50 border-cyan-400/30 text-white text-xs">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {engines.map(eng => (
                    ...
                    <AeroFormField label="Count" helperText="" htmlFor={`count-${stageIndex}-${engineIndex}`}>
                      <Input
                        id={`count-${stageIndex}-${engineIndex}`}
                        type="number"
                        value={engine.count}
                        onChange={(e) => updateStageEngine(stageIndex, engineIndex, 'count', parseInt(e.target.value) || 1)}
                        className="bg-slate-600/50 border-cyan-400/30 text-white text-xs"
                        min="1"
                        step="1"
                      />
                    </AeroFormField>
                    ...
                    <AeroFormField label="Isp" helperText="s" htmlFor={`isp-${stageIndex}-${engineIndex}`}>
                      <Input
                        id={`isp-${stageIndex}-${engineIndex}`}
                        type="number"
                        value={engine.isp}
                        onChange={(e) => updateStageEngine(stageIndex, engineIndex, 'isp', parseFloat(e.target.value) || 0)}
                        className="bg-slate-600/50 border-cyan-400/30 text-white text-xs"
                        min="0"
                        step="10"
                      />
                    ...
                    <div className="flex items-end">
                      <Button
                        onClick={() => removeEngine(stageIndex, engineIndex)}
                        variant="ghost"
                        size="sm"
                        className="text-red-400 hover:text-red-300"
